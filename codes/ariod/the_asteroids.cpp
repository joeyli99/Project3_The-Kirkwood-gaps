...
int main()
{
	...
	std::uniform_real_distribution<double> u(0, 2*PI);
    //std::uniform_int_distribution<int> year(6000, 8000);
    int years = 2000;
    char input_file[256]={0};
    char output_file[256]={0};
    sprintf_s(input_file, "IV%d-%d-%d%d-[(%d)_(%d)]randangle.txt", 1+pTm->tm_mon, pTm->tm_mday,pTm->tm_hour,pTm->tm_min, years, n);
    sprintf_s(output_file,"DATA%d-%d-%d%d-[(%d)_(%d)]randangle.txt", 1+pTm->tm_mon, pTm->tm_mday,pTm->tm_hour,pTm->tm_min, years, n);
	...
	for (int i = 0; i < n; i++)
	{
		...
		//cout<<phi<<endl;
		std::vector<double> q0 = { 5.4588, 0, r*cos(phi), r*sin(phi) };
		std::vector<double> p0 = { 0, 2.62267, -sqrt(4 * PI * PI / r)*sin(phi), sqrt(4 * PI * PI / r)*cos(phi) };
